...
const Card = ( {name,salary,myarray , btnText = "VistMe"} ) => {
  return (
    <div className="md:max-w-sm w-full p-6 rounded-xl shadow-xl bg-blue-700 border border-zinc-800 hover:border-white transition-all duration-300 hover:transform hover:scale-105 group">
        {myarray.map((item,index)=>{
            return <b className="p-2" key={index}>{item}</b>
        })}
